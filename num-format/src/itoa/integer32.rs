...
impl super::Integer for i32 {
    ...
    fn write<'a>(self, buf: &mut Self::Buffer) -> &'a str {
        ...
        let n = if is_nonnegative {
            ...
            (!(self as u32)).wrapping_add(1)
        };

        let buf_ptr = buf.0;
        let buf_len = buf.1;
        let mut curr = write_u32(n, buf_ptr, buf_len) as isize;
...
impl super::Integer for u32 {
    ...
    fn write<'a>(self, buf: &mut Self::Buffer) -> &'a str {
        let buf_ptr = buf.0;
        let buf_len = buf.1;
        let curr = write_u32(self, buf_ptr, buf_len) as isize;
...
#[inline(never)]
fn write_u32(mut n: u32, buf: *mut u8, pos: usize) -> usize {
    let mut curr = pos as isize;
    let lut_ptr = DIGIT_TABLE.as_ptr();

    unsafe {
        // need at least 16 bits for the 4-characters-at-a-time to work.
        if mem::size_of::<u32>() >= 2 {
